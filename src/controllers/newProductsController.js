const newProdModel = require('../models/newProductModel')
...
module.exports = {
  getProducts: async (req, res) => {
    ...
    const count = await newProdModel.countModel([searchKey, searchValue, sortKey, sortBy])
    const page = paging(req, count[0].count)
    const { offset, pageInfo } = page
    const { limitData: limit } = pageInfo

    const results = await newProdModel.getModel([searchKey, searchValue, sortKey, sortBy], [limit, offset])
    if (results.length) {
  ...
  detailProduct: async (req, res) => {
    ...
    const results = await newProdModel.detailModel(id)
    if (results.length) {
