const usersAddressModel = require('../models/usersAddressModel')
const responseStandard = require('../helpers/response')
const { addressSchema: schema } = require('../helpers/validation')
...
module.exports = {
  createAddress: async (req, res) => {
    ...
    if (error) {
      ...
      const create = await usersAddressModel.createModel(data)
  ...
  getAllAddress: async (req, res) => {
    const { id } = req.data
    const count = await usersAddressModel.countModel(id)
    const page = paging(req, count[0].count)
    const { offset, pageInfo } = page
    const { limitData: limit } = pageInfo

    const results = await usersAddressModel.getModel([id, limit, offset])
    if (results.length) {
  ...
  detailAddress: async (req, res) => {
    ...
    const results = await usersAddressModel.detailModel([userId, id])
    if (results.length) {
  ...
  updateAll: async (req, res) => {
    ...
    if (error) {
      ...
      const isExist = await usersAddressModel.detailModel([userId, id])
      if (isExist.length) {
        const { name, recipientName, recipientPhone, address, postalCode, city } = results
        const data = {
          name,
        ...
        const update = await usersAddressModel.updateModel([data, id])
  ...
  updatePartial: async (req, res) => {
    ...
    if (isExist.length) {
      ...
      console.log(data)
      const update = await usersAddressModel.updatePartial(data, parseInt(id))
      if (update.affectedRows) {
        return responseStandard(res, 'Update address successfully')
      } else {
        return responseStandard(res, 'Failed to update', {}, 500, false)
  ...
  deleteAddress: async (req, res) => {
    ...
    const isExist = await usersAddressModel.detailModel([userId, id])
    if (isExist.length) {
      const deleteAdd = await usersAddressModel.deleteModel([userId, id])
